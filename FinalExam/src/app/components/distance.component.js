"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var core_1 = require("@angular/core");
var DisComponent = (function () {
    function DisComponent() {
    }
    DisComponent.prototype.getWeather = function () {
        x = '20';
        document.getElementById("dist").innerHTML = "Hello Angular";
        var json = getJSON('https://maps.googleapis.com/maps/api/distancematrix/json?origins=Toronto&destinations=Montreal', function (err, data) {
            if (err != null) {
                console.log("Failed to read json data");
            }
            else {
                update_dist(data);
            }
        });
    };
    DisComponent.prototype.update_dist = function (data) {
    };
    return DisComponent;
}());
DisComponent = __decorate([
    core_1.Component({
        selector: 'distance',
        template: "\n     <h1>The Distance between Toronto and Mantreal is {{x}}. It will take {{y}} to get there.</h1>\n   ",
    })
], DisComponent);
exports.DisComponent = DisComponent;
function getJSON(url, callback) {
    var xhr = new XMLHttpRequest();
    xhr.open("get", url, true);
    xhr.responseType = "json";
    xhr.onload = function () {
        var status = xhr.status;
        if (status == 200) {
            callback(null, xhr.response);
        }
        else {
            callback(status);
        }
    };
    xhr.send();
}
;
//# sourceMappingURL=distance.component.js.map